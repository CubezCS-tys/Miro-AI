...
import { executeCode, type CodeResult } from "@/lib/api";
...
export function CodeNode({ id, data, selected }: NodeProps<CodeNodeType>) {
  ...
  }, [running, runtime, data.code, id, updateNodeData]);
    ...
    >
      ...
      <div className="flex shrink-0 cursor-grab items-center gap-2 border-b border-white/8 px-3 py-2 active:cursor-grabbing">
        ...
        <button
          ...
          className="nodrag rounded-md bg-white/5 px-2 py-0.5 text-[10px] font-medium text-slate-400 transition-colors hover:bg-white/10 hover:text-slate-200"
          title={
            runtime === "browser"
              ? "Runs in your browser (WebAssembly sandbox) — click to use the server's Python"
              : "Runs on the backend's real Python — click to use the browser sandbox"
          }
        >
